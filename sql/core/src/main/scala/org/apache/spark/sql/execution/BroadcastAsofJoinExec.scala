...
import org.apache.spark.sql.catalyst.expressions.{Ascending, Attribute, AttributeSet, Expression, GenericInternalRow, JoinedRow, SortOrder, UnsafeProjection, UnsafeRow}
...
case class BroadcastAsofJoinExec(
  ...
  override val output: Seq[Attribute] = left.output ++ right.output.map(_.withNullability(true))

  override def outputPartitioning: Partitioning = buildSide match {
    case BuildLeft => right.outputPartitioning
    case BuildRight => left.outputPartitioning
  }
